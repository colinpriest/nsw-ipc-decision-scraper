"""
CTP-only export: rows with both Impairment % and Lump Sum, enriched with every
...
if filtered.empty:
    raise SystemExit("No CTP rows with numeric Impairment % and Lump Sum.")

enriched_records = [enrich_row(url) for url in filtered["URL"]]
enriched_df = pd.DataFrame(enriched_records, index=filtered.index).fillna("")
out_df = pd.concat([filtered, enriched_df], axis=1)

# Column ordering: keep the existing flat layout, then narrative, then slices,
# then key paragraphs + event history.
new_cols = (
...
# Coerce known-numeric LLM fields to actual numbers in Excel. Values like
# "Not stated", "Nil", "Not applicable" become NaN -> empty cell. Lump Sum
# and Impairment % already coerce naturally because the filter strips
# non-numeric rows; these other money/numeric fields don't, because we keep
# rows where they're absent.
NUMERIC_COERCE_COLUMNS = [
    ...
    # so any future stringy "" cells become NaN instead of blocking sums.
...
# Use the shared tolerant coercer from nsw_court_scraper so this script
...
# For the Excel: the analysis-useful WPI is Impairment % (Accepted), which is
# populated for every filtered row. The strict Impairment % field is only
# populated for ~21% of CTP cases and creates the misleading impression of
# missing data. Drop it, rename Accepted -> "WPI %" (which is what the field
# is normally called), and move it adjacent to Lump Sum for visibility.
...
if "WPI %" in out_df.columns and "Lump Sum" in out_df.columns:
    ...
    cols.remove("WPI %")
    insert_at = cols.index("Lump Sum") + 1
    cols.insert(insert_at, "WPI %")
    out_df = out_df[cols]

out_df.to_excel(OUTPUT_XLSX, index=False)
print(f"Wrote {len(out_df)} rows × {len(out_df.columns)} cols to {OUTPUT_XLSX}")
print(f"  Source CSV: {INPUT_CSV}")
print(f"  Sidecar:    {'loaded ' + str(len(sidecar)) + ' entries' if sidecar else 'NOT FOUND'}")
print(f"  New cols added: {len(new_cols)} (narrative={len(NARRATIVE_FIELDS)}, "
